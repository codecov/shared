...
class Gitlab(BaseHandler):
    ...
    async def api(self, method, path, body=None, token=None, **args):
        ...
        try:
            ...
            if e.code == 599:
                ...
                raise TorngitServerUnreachableError(
                    "Gitlab3 was not able to be reached, server timed out."
                )
            elif e.code >= 500:
    ...
    async def list_repos(self, username=None, token=None):
        ...
        while True:
            ...
            repos = await self.api(
                "get", "/projects?per_page=100&page=%d" % page, token=token
            )
            for repo in repos:
                ...
                data.append(
                    dict(
                        ...
                        repo=dict(
                            ...
                            language=None,
                            branch=(repo["default_branch"] or "master").encode(
                                "utf-8", "replace"
                            ),
                        ),
                    )
    ...
    async def list_teams(self, token=None):
        ...
        groups = await self.api("get", "/groups")
        return [dict(name=g["name"], id=g["id"], username=g["path"]) for g in groups]

    async def get_pull_request(self, pullid, token=None):
        # https://docs.gitlab.com/ce/api/merge_requests.html#get-single-mr
        pull = await self.api(
            "get",
            "/projects/{}/merge_requests?iid={}".format(
        ...
        if pull:
            ...
            return dict(
                base=dict(
                    branch=(pull["target_branch"] or "").encode(
                        "utf-8", "replace", commitid=parent
                    ),
                    head=dict(
                        branch=(pull["source_branch"] or "").encode("utf-8", "replace"),
                        commitid=pull["sha"],
                    ),
                    state="open"
    ...
    async def set_commit_status(
        ...
        res = await self.api(
            ...
            body=dict(
                state=status, target_url=url, name=context, description=description
            ),
            token=token,
        )
    ...
    async def get_commit_statuses(self, commit, _merge=None, token=None):
        ...
        _states = dict(
            pending="pending", success="success", error="failure", failure="failure"
        )
        statuses = [
            {
    ...
    async def post_comment(self, issueid, body, token=None):
        ...
        res = await self.api(
            ...
            % (self.data["repo"]["service_id"], issueid),
            body=dict(body=body),
            token=token,
        )
        return res["id"]
    ...
    async def edit_comment(self, issueid, commentid, body, token=None):
        ...
        await self.api(
            ...
            % (self.data["repo"]["service_id"], issueid, commentid),
            body=dict(body=body),
            token=token,
        )
        return commentid
    ...
    async def get_pull_request_commits(self, pullid, token=None):
        ...
        commits = await self.api(
            "get",
            "/projects/{0}/merge_requests/{1}/commits".format(
                self.data["repo"]["service_id"], pullid
            ),
            token=token,
        )
    ...
    async def get_pull_requests(self, state="open", token=None):
        ...
        merge_request_url = (
            "/projects/%s/merge_requests/{0}/commits" % self.data["repo"]["service_id"]
        )
    ...
    async def find_pull_request(
        ...
        if commit:
            ...
            for pull in res:
                if (
                    pull["source_branch"]
                    and pull["source_branch"].encode("utf-8", "replace") == branch
                ):
                    return pull["iid"]
    ...
    async def get_is_admin(self, user, token=None):
        ...
        res = await self.api(
            "get", "/groups/%s/members" % self.data["owner"]["service_id"], token=token
        )
        res = any(
            filter(lambda u: u and (u["id"] == user_id and u["access_level"] > 39), res)
        )
        return res
