...
class Report:
    ...
    _header: ReportHeader = ReportHeader(labels_index={})
        ...
        **kwargs,
        ...
        if files:
            for name, summary in files.items():
                ...
                try:
                    lines = _chunks[chunks_index].decode()
                except IndexError:
                    lines = ""

                # TODO: do something with `file_diff_totals`
                self._files[name] = ReportFile(name, totals=file_totals, lines=lines)
    ...
    def to_archive(self, with_header=True):
        ...
        chunks = END_OF_CHUNK.join(
            _encode_chunk(chunk).decode() for chunk in self._files
        )
        if with_header:
