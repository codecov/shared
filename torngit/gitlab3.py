...
class Gitlab(BaseHandler):
    ...
    async def get_pull_requests(self, state='open', token=None):
        ...
        merge_request_url = '/projects/%s/merge_requests/{0}/commits' % self.data[
            'repo']['service_id']
    ...
    async def get_is_admin(self, user, token=None):
        ...
        res = await self.api(
            'get',
            '/groups/%s/members' % self.data['owner']['service_id'],
            token=token)
        res = any(
            filter(
                lambda u: u and (u['id'] == user_id and u['access_level'] > 39),
                res))
        return res
