...
class BitbucketServer(BaseHandler):
    ...
    urls = dict(
        ...
        src=
        'projects/%(username)s/repos/%(name)s/browse/%(path)s?at=%(commitid)s',
        tree='projects/%(username)s/repos/%(name)s/browse?at=%(commitid)s',
        create_file=None,
        branch='projects/%(username)s/repos/%(name)s/browser?at=%(branch)s',
        pull=
        'projects/%(username)s/repos/%(name)s/pull-requests/%(pullid)s/overview',
        compare='')
    ...
    def diff_to_json(self, diff_json):
        ...
        for _diff in diff_json:
            if not _diff.get('destination'):
                ...
                _before = _diff['source']['toString'] if _diff.get(
                    'source') else None
    ...
    async def api(self, method, url, body=None, token=None, **kwargs):
        ...
        if self.verify_ssl is False:
            ...
            verify_ssl = dict(
                disable_ssl_certificate_validation=True, ca_certs=False)
        ...
        client = oauth.Client(
            oauth.Consumer(self._oauth_consumer_token()['key'], ''), token,
            **verify_ssl)
    ...
    async def get_repository(self, token=None):
        ...
        if res.get('origin'):
            ...
            if res['origin']['project']['type'] == 'PERSONAL':
                _fork_owner_service_id = 'U%d' % res['origin']['project'][
                    'owner']['id']
        ...
        return (dict(
            owner=dict(
                service_id=owner_service_id, username=res['project']['key']),
            repo=dict(
                service_id=res['id'],
                language=None,
                private=(not res.get('public',
                                     res.get('origin', {}).get('public'))),
                branch='master',
                fork=fork,
                name=res['slug'])))
    ...
    async def get_commit(self, commit, token=None):
        ...
        res = await self.api(
            'get',
            '%s/repos/%s/commits/%s' % (self.project,
                                        self.data['repo']['name'], commit),
            token=token)
        ...
        if not author['size']:
            author = await self.api(
                'get', '/users', filter=res['author']['name'])
        ...
        return (dict(
            ...
            timestamp=datetime.fromtimestamp(
                int(str(res['authorTimestamp'])[:10])).strftime(
                    '%Y-%m-%d %H:%M:%S')))
    ...
    async def get_compare(self,
        ...
        diff = (await self.api(
            'get',
            '%s/repos/%s/commits/%s/diff' % (self.project,
                                             self.data['repo']['name'], head),
            withComments=False,
            whitespace='ignore-all',
            contextLines=context or -1,
            since=base,
        ...
        while with_commits:
            ...
            res = await self.api(
                'get',
                '%s/repos/%s/commits' % (self.project,
                                         self.data['repo']['name']),
                start=start,
                token=token,
                since=base,
    ...
    async def get_pull_request(self, pullid, token=None):
        ...
        return (dict(
            ...
            number=str(pullid),
            base=dict(
                branch=res['toRef']['id'].encode('utf-8', 'replace').replace(
                    'refs/heads/', ''),
                commitid=first_commit),
            head=dict(
                branch=res['fromRef']['id'].encode('utf-8', 'replace').replace(
                    'refs/heads/', ''),
    ...
    async def list_repos(self, username=None, token=None):
        ...
        while True:
            ...
            for repo in res['values']:
                ...
                fork = None
                if repo.get('origin'):
                    _fork_owner_service_id = str(
                        repo['origin']['project']['id'])
                    if repo['origin']['project']['type'] == 'PERSONAL':
                        _fork_owner_service_id = 'U%d' % repo['origin'][
                            'project']['owner']['id']
                ...
                data.append(
                    dict(
                        ...
                        repo=dict(
                            ...
                            language=None,
                            private=(not repo.get(
                                'public',
                                repo.get('origin', {}).get('public'))),
                            branch='master',
                            fork=fork)))
            if res['isLastPage'] or res.get('nextPageStart') is None:
                break
    ...
    async def get_commit_statuses(self, commit, _merge=None, token=None):
        ...
        while True:
            res = await self.api(
                'get',
                '%s/rest/build-status/1.0/commits/%s' % (self.service_url,
                                                         commit),
                start=start,
                token=token)
            if len(res['values']) == 0:
    ...
    async def get_branches(self, token=None):
        ...
        while True:
            ...
            res = await self.api(
                'get',
                '%s/repos/%s/branches' % (self.project,
                                          self.data['repo']['name']),
                start=start,
                token=token)
            if len(res['values']) == 0:
                break
            branches.extend([(b['displayId'].encode('utf-8', 'replace'),
                              b['latestCommit']) for b in res['values']])
            if res['isLastPage'] or res.get('nextPageStart') is None:
                break
            else:
                start = res['nextPageStart']
    ...
    async def find_pull_request(self,
        ...
        if branch:
            ...
            while True:
                ...
                res = await self.api(
                    'get',
                    '%s/repos/%s/pull-requests' % (self.project,
                                                   self.data['repo']['name']),
                    state=state,
                ...
                for pull in res['values']:
                    if pull['fromRef']['id'].encode('utf-8',
                                                    'replace').replace(
                                                        'refs/heads/',
                                                        '') == branch:
                        return (pull['id'])
