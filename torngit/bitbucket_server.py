...
class BitbucketServer(BaseHandler):
    ...
    def set_commit_status(self, commit, status, context, description, url=None, merge_commit=None, token=None, coverage=None):
        ...
        yield self.api('post', '%s/rest/build-status/1.0/commits/%s' % (self.service_url, commit),
                       ...
                       token=token)
        if merge_commit:
            yield self.api('post', '%s/rest/build-status/1.0/commits/%s' % (self.service_url, merge_commit[0]),
                           body=dict(state=dict(pending='INPROGRESS', success='SUCCESSFUL', error='FAILED', failure='FAILED').get(status),
                                     key=merge_commit[1],
                                     name=merge_commit[1],
                                     url=url,
        ...
        if branch:
            ...
            while True:
                ...
                for pull in res['values']:
                    if pull['fromRef']['id'].encode('utf-8', 'replace').replace('refs/heads/', '') == branch:
                        raise gen.Return(pull['id'])
