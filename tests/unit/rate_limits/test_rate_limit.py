...
from shared.rate_limits import set_entity_to_rate_limited, determine_entity_redis_key, determine_if_entity_is_rate_limited
...
class TestRateLimits(object):
    ...
    def test_determine_entity_redis_key_installed_gh_app_via_repository(self, mock_get_github_integration_token):
        owner = OwnerFactory(
            service="github",
            bot=None,
        ...
        repository = RepositoryFactory(
            author=owner, using_integration=None
        )
        repository.save()
        assert determine_entity_redis_key(owner=owner, repository=repository) == f"{gh_app.app_id}_{gh_app.installation_id}"
    ...
    @pytest.mark.django_db(databases={"default"})
    def test_determine_entity_redis_key_installed_gh_app_via_owner(self, mock_get_github_integration_token):
        owner = OwnerFactory(
            service="github",
            bot=None,
            unencrypted_oauth_token="owner_token: :refresh_token",
    ...
    def test_determine_entity_redis_key_owner_token_via_repository(self):
        ...
        repository.save()
        assert determine_entity_redis_key(owner=owner, repository=repository) == str(repository.author.ownerid)
    ...
    def test_determine_entity_redis_key_owner_token_via_owner(self):
        ...
        owner.save()
        repository = None
        assert determine_entity_redis_key(owner=owner, repository=repository) == str(owner.ownerid)

    def test_determine_if_entity_is_rate_limited_true(self):
        key_name = "owner_id_123"
        set_entity_to_rate_limited(redis_connection=self.redis_connection, key_name=key_name, ttl_seconds=300)
        assert determine_if_entity_is_rate_limited(redis_connection=self.redis_connection, key_name=key_name) == True

    def test_determine_if_entity_is_rate_limited_false(self):
        assert determine_if_entity_is_rate_limited(redis_connection=self.redis_connection, key_name="random_non_existent_key") == False
