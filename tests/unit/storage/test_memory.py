...
class TestMemoryStorageService(BaseTestCase):
    ...
    def test_list_folder_contents(self, request, codecov_vcr):
        ...
        assert sorted(expected_result_1, key=lambda x: (x['name'], x['size'])) == sorted(results_1, key=lambda x: (x['name'], x['size']))
        ...
        assert sorted(expected_result_2, key=lambda x: (x['name'], x['size'])) == sorted(results_2, key=lambda x: (x['name'], x['size']))
