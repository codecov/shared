...
def test_report():
    paths = ['a', 'b']
    report = Mock(filter=Mock(return_value=Mock(__enter__=lambda self: self, __exit__=Mock())), yaml={'flags': {'a': {'paths': paths}}})
...
def test_totals():
    report = Mock(filter=Mock(return_value=Mock(__enter__=lambda self: self, __exit__=Mock(), totals='<totals>')), yaml={})
    assert Flag(report, '_').totals == '<totals>'


def test_apply_diff():
    apply_diff = Mock(return_value='<totals>')
    report = Mock(filter=Mock(return_value=Mock(__enter__=lambda self: self, __exit__=Mock(), apply_diff=apply_diff)), yaml={})
