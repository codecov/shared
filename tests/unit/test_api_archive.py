...
def mock_config(mocker):
    ...
    mock_get_config.side_effect = lambda *args, default=None: {
        ...
        ("services", "minio", "hash_key"): "test-key",
    }.get(args, default)
...
class TestArchiveService:
    ...
    def test_read_chunks(self, archive_service):
    ...
    def test_create_presigned_put(self, archive_service):
        result = archive_service.create_presigned_put("test/path")

        assert isinstance(result, str)
        assert len(result) > 0

    def test_write_json_data_to_storage_with_commit(self, archive_service):
        data = {"key": "value"}
    ...
    def test_write_json_data_to_storage_without_commit(self, archive_service):
        data = {"key": "value"}
    ...
    def test_write_json_data_to_storage_custom_encoder(self, mocker, archive_service):
