...
class Test(AsyncTestCase):
    ...
    def make_commit(self):
        ...
        sha = requests.post(
            "https://api.github.com/repos/torngit/ci/git/commits",
            headers=self.headers,
            data=
            '{"message":"ci","tree":"25fc45bb68028118bdc6f65be185784858f339b6","parents":["99b53601f852ecc70d50dd10e172f6b7f101bbbc"]}'
        ).json()['sha']
        return sha
    ...
    def make_merge_commit(self):
        ...
        sha2 = requests.post(
            ...
            headers=self.headers,
            data=
            '{"message":"Merge %s into %s","tree":"25fc45bb68028118bdc6f65be185784858f339b6","parents":["99b53601f852ecc70d50dd10e172f6b7f101bbbc"]}'
            % (sha1, 'a' * 40)).json()['sha']
        return sha2, sha1
    ...
    def test_get_pull_requests_at_sha(self):
        res = yield self.gh.get_pull_requests(
            '9e0ac7c916adc0573014f1d379bdf7ef45acd23e')
        assert res == [(None, '1')]
    ...
    def test_get_pull_request(self):
        ...
        assert res == {
            ...
            'number': '1',
            'base': {
                'commit': u'99b53601f852ecc70d50dd10e172f6b7f101bbbc',
                'branch': u'master'
            },
            'head': {
                'commit': u'9e0ac7c916adc0573014f1d379bdf7ef45acd23e',
                'branch': u'pr'
            }
        }
    ...
    def test_get_pull_request_merge_commit(self):
        res = yield self.gh.get_pull_requests(
            '6b7cf45238ec409064893b51f8dfa2f3ce51c99c')
        assert res == [(None, '1')]
    ...
    def test_get_merge_commit_head_none(self):
        res = yield self.gh._get_merge_commit_head(
            '99b53601f852ecc70d50dd10e172f6b7f101bbbc')
        assert res is None
    ...
    def test_get_source(self):
        res = yield self.gh.get_source(
            'readme.md', '99b53601f852ecc70d50dd10e172f6b7f101bbbc')
    ...
    def test_get_commit_status(self):
        res = yield self.gh.get_commit_status(
            '99b53601f852ecc70d50dd10e172f6b7f101bbbc')
        assert res == 'pending'

    @gen_test
    def test_get_commit_statuses(self):
        res = yield self.gh.get_commit_statuses(
            '99b53601f852ecc70d50dd10e172f6b7f101bbbc')
        self.assertItemsEqual(res._statuses,
                              [{
                                  'url': None,
                                  'state': u'pending',
                                  'context': u'other',
                                  'time': u'2015-12-21T16:54:13Z'
                              },
                               {
                                   'url': None,
                                   'state': u'success',
                                   'context': u'ci',
                                   'time': u'2015-12-21T16:54:05Z'
                               }])

    @gen_test
    def test_get_commit_statuses_merge_commit(self):
        sha2 = yield self.gh.get_commit_statuses(
            '6b7cf45238ec409064893b51f8dfa2f3ce51c99c')
        assert sha2 is None
    ...
    def test_set_commit_status(self):
        ...
        print(sha)
        res = yield self.gh.set_commit_status(sha, 'success', 'torngit/demo',
                                              'Hello world')
        assert res
        res = requests.get(
            'https://api.github.com/repos/torngit/ci/commits/%s/statuses' %
            sha,
    ...
    def test_post_comment(self):
        ...
        assert int(cid)
        res = requests.get(
            'https://api.github.com/repos/torngit/ci/issues/comments/%s' % cid,
            headers=self.headers).json()
        assert res['body'] == r
        requests.delete(
            'https://api.github.com/repos/torngit/ci/issues/comments/%s' % cid,
    ...
    def test_edit_comment(self):
        ...
        assert res
        res = requests.get(
            'https://api.github.com/repos/torngit/ci/issues/comments/166593320',
            headers=self.headers).json()
        assert res['body'] == r
    ...
    def test_create_webhook(self):
        ...
        for hook in requests.get(
                'https://api.github.com/repos/torngit/ci/hooks',
                headers=self.headers).json():
            if 'localhost' in hook['config'].get('url', ''):
                # https://developer.github.com/v3/repos/hooks/#delete-a-hook
                requests.delete(
                    'https://api.github.com/repos/torngit/ci/hooks/%s' % str(
                        hook['id']),
                    headers=self.headers)

        hook = yield self.gh.post_webhook('http://localhost', ['push'],
                                          'abc123')
        assert hook is not None
        # https://developer.github.com/v3/repos/hooks/#get-single-hook
        res = requests.get(
            'https://api.github.com/repos/torngit/ci/hooks/%s' % str(hook),
            headers=self.headers)
    ...
    def test_list_teams(self):
        ...
        gh = Github(
            ...
            None,
            token=dict(
                key='testgeghyogc7wjiscwy9ms1fjv3q4ds1y7s1dcv',
                secret='',
                username='codecov-test'),
            ioloop=self.io_loop)
    ...
    def test_get_authenticated_404(self):
        with self.assertRaises(HTTPError):
            yield Github(
                None, 'torngit', '404',
                ioloop=self.io_loop).get_authenticated()
        ...
        assert repos == [{
            ...
            'repo_service_id': 48378290
        },
                         {
                             'fork': {
                                 'username': u'codecov',
                                 'repo': u'example-python',
                                 'branch': u'master',
                                 'owner_service_id': 8226205,
                                 'private': False,
                                 'repo_service_id': 24344106
                             },
                             'username': u'torngit',
                             'branch': u'master',
                             'repo': u'example-python',
    ...
    def test_get_branches(self):
        ...
        assert res == [(u'closedpr',
                        u'5061da92b04ce307e38cf84a443c36d8831dee45'),
                       (u'master',
                        u'6b7cf45238ec409064893b51f8dfa2f3ce51c99c'),
