...
from covreports.reports.types import ReportTotals, ReportLine
...
def test_get_slice():
    ...
    r = ReportFile(
        name="folder/file.py",
        lines=[ReportLine(1), ReportLine(2), ReportLine(3), ReportLine(4)],
...
@pytest.mark.parametrize(
    ...
    [
        (
            ReportFile(
                name="folder/file.py",
                lines=[ReportLine(1), ReportLine(0)],
                line_modifier=lambda l: None,
            ),
            [],
        ),
...
def test_report_iter():
    ...
    r = ReportFile(
        name="folder/file.py",
        lines=[ReportLine(coverage=1), ReportLine(coverage=0)],
        line_modifier=filter_lines,
    )
    lines = []
    for ln in r:
